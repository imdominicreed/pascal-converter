...
public class Converter extends PascalBaseVisitor<Object>
{
    ...
    public Object visitForStatement(PascalParser.ForStatementContext ctx) { 
        ...
        code.emit(variable);
        code.emit(" = ");
        code.emit((String) visit(ctx.expression(0)));
        code.emit("; ");
        code.emit(variable);
        code.emit(" " + (ctx.DOWNTO() == null ? "<= " : ">= "));
        code.emit((String) visit(ctx.expression(1)));
        code.emit("; ");
        code.emit(variable + (ctx.DOWNTO() == null ? "++" : "--") + ") ");
        visit(ctx.statement());
        return null;
    }
    ...
    public Object visitCaseStatement(PascalParser.CaseStatementContext ctx) {
        ...
        //   visit(ctx.caseBranchList().caseBranch(i);
    ...
    public Object visitCaseBranch(PascalParser.CaseBranchContext ctx) {
        if(ctx.caseConstantList() == null) return null;
        for(int i = 0; i < ctx.caseConstantList().caseConstant().size(); i++) {
            var branch = ctx.caseConstantList().caseConstant(i);
            code.emitStart("case " + branch.getText() + ": ");
        }
        code.indent();
        code.emitStart("");
        visit(ctx.statement());
    ...
    public Object visitProcedureCallStatement(PascalParser.ProcedureCallStatementContext ctx) {
        ...
        if(ctx.argumentList() != null) {
            ...
            for (var argCtx : ctx.argumentList().argument()) {
                sb.append(argCtx.getText()).append(", ");
            }
            code.emit(sb.delete(sb.length()-2, sb.length()).toString());
        }
        code.emit(");");
